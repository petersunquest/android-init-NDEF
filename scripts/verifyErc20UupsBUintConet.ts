/**
 * CoNET Blockscout 验证 BeamioBUnits UUPS 栈（impl Standard JSON + proxy legacy partial）。
 *
 * 运行:
 *   npm run clean && npm run compile
 *   npx tsx scripts/exportBeamioBUnitsUupsStandardJson.ts
 *   npx tsx scripts/verifyErc20UupsBUintConet.ts
 */
...
const IMPL_CONTRACT = "project/src/b-unit/BUint.sol:BeamioBUnits";
...
async function getVerified(addr: string): Promise<boolean> {
  const res = await fetch(`${BLOCKSCOUT_UI.replace(/\/$/, "")}/api/v2/smart-contracts/${addr}`);
...
async function verifyImpl(impl: string): Promise<void> {
  ...
  const jsonPath = path.join(root, "deployments/base-BeamioBUnits-standard-input-FULL-FORM.json");
  if (!fs.existsSync(jsonPath)) {
    throw new Error(`缺少 ${jsonPath}，请先 exportStandardJsonFromBuildInfo.mjs BeamioBUnits --full 并剪枝 FORM`);
  }
...
async function verifyProxy(proxy: string, impl: string): Promise<void> {
  ...
  const iface = new Interface(["function initialize(address)"]);
  const initData = iface.encodeFunctionData("initialize", [BUINT_INITIAL_ADMIN]);
  ...
  if (!fs.existsSync(ozJson)) {
    const initCode = buildErc1967ProxyInitCode(impl, initData);
    console.warn("缺少 oz-ERC1967Proxy-standard-input.json，proxy 验证需手工上传 OZ ERC1967Proxy JSON");
...
async function main() {
  const impl = getAddress(process.env.BUINT_UUPS_IMPL || BUINT_UUPS_IMPL_PREDICTED);
  const proxy = getAddress(process.env.BUINT_UUPS_PROXY || BUINT_UUPS_PROXY_PREDICTED);
  console.log("impl:", impl);
  console.log("proxy:", proxy);
  await verifyImpl(impl);
  await verifyProxy(proxy, impl);
  console.log(`Explorer: ${BLOCKSCOUT_UI}/address/${proxy}#code`);
}
