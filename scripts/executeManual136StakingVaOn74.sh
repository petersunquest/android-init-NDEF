#!/usr/bin/env bash
# Manual 136 staking validators on 74.208.224.45 — NO Guardian / DePIN / claimRedeem / mining-pool slot binding.
# NO contract upgrade: withdrawNative (contract admin) + direct beacon deposit() txs.
#
# Model:
#   - NEW validator keys on 74 (do not reuse legacy exited keys)
#   - Contract admin withdrawNative → deposit_sender wallet; then 136× deposit() @ 32 CNET
...
echo "==> [1/6] Rsync validator-node scripts to ${HOST74}"
rsync -av \
	...
	"${SSH74}:${NEWCONET}/"
ssh "$SSH74" "chmod +x '${NEWCONET}/01_generate_append_validator_deposits_listener.sh' '${NEWCONET}/08_import_append_validator_keys.sh' '${NEWCONET}/setConetLabMiningPoolFeeRecipient74.sh' '${NEWCONET}/batchExitValidatorsFromDeposits.sh' '${NEWCONET}/06_exit_validator.sh'"

echo "==> [2/6] Voluntary-exit legacy validators on ${HOST74} (if VA still uses old keys)"
...
echo "==> [3/6] Generate ${COUNT} NEW validator keys (wc=ValidatorDepositRedeem) on ${HOST74}"
ssh "$SSH74" bash -s -- "$NEWCONET" "$REDEEM" "$COUNT" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
REDEEM="$2"
COUNT="$3"
...
echo "==> [5/6] Import keystores on ${HOST74}"
...
echo "==> [6/6] proposer-settings (ConetLabMiningPool) + restart VA on ${HOST74}"
ssh "$SSH74" bash -s -- "$NEWCONET" "$POOL" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
POOL="$2"
...
echo "==> Done. 74 runs manual ${COUNT} VA: Redeem wc, ConetLabMiningPool fee_recipient, no guardian ledger."
