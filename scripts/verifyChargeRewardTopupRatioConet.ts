/**
 * Blockscout verify ChargeRewardModuleV2 from conet-ChargeRewardTopupRatio.json.
 *
 * Prerequisites:
...
const snapPath = path.join(root, 'deployments/conet-ChargeRewardTopupRatio.json')
if (!fs.existsSync(snapPath)) {
	throw new Error(`Missing ${snapPath} — run upgradeChargeRewardTopupRatioConet.ts first`)
}
const snap = JSON.parse(fs.readFileSync(snapPath, 'utf-8')) as {
	modules: { chargeRewardModule: string }
	libraryLinks: {
