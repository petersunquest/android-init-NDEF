#!/usr/bin/env node
/**
 * Verify GlobalArchiveRoutingRegistryV1 impl + DLEERC1967Proxy on CoNET Blockscout.
 *
 * Local solc deployedBytecode must equal eth_getCode before any submit.
 * Impl and this-repo proxy use v2 standard-input after that preflight.
 */
...
const recordPath = path.join(root, "deployments/conet-GlobalArchiveRoutingRegistry.json");
...
const pollAttempts = Number(process.env.CONET_VERIFY_POLL_MAX ?? "180");
const DLE_SOLC_VERSION = "0.8.35";

function fail(message) {
  throw new Error(`Global routing verification failed: ${message}`);
}
...
function resolveImport(fromKey, specifier, sources) {
  ...
  if (specifier.startsWith("@")) {
    const direct = Object.keys(sources).find((key) => normalizeNpmSourceKey(key) === specifier);
...
function pruneFullBuildInfo(fullInput, rootKey) {
  ...
  while (stack.length > 0) {
    ...
    keep.add(current);
    const imports = source.content.matchAll(/import\s+(?:[^'"]*\s+from\s+)?["']([^"']+)["']/g);
...
function loadExactBuildInfo(component) {
  ...
  if (buildInfo.solcVersion !== DLE_SOLC_VERSION) {
    fail(`${component.key} was not compiled with required solc ${DLE_SOLC_VERSION}`);
  }
  const onDisk = path.join(root, component.sourceKey.replace(/^project\//, ""));
...
async function submitStandardJson(target, standardJson, compilerVersion, constructorArgs) {
  ...
  if (!response.ok && !/already verified/i.test(body)) {
    fail(`${target.key} Blockscout v2 submission failed (HTTP ${response.status}): ${body.slice(0, 500)}`);
  }
}
...
async function probeSolcTail(provider, address) {
  const code = await provider.getCode(address);
  return { code, tail: code.slice(-24) };
}

async function main() {
  if (!skipCompile) runCleanCompile();
  const record = readJson(recordPath);
  if (record.schema !== "conet-dle-global-routing-v1" || record.recordState !== "deployed") {
    fail("record must be conet-dle-global-routing-v1 with recordState=deployed");
  }
  if (record.chainId !== 224422) fail("record chainId must be 224422");
  if (!Array.isArray(record.components) || record.components.length !== 2) {
    fail("record must contain impl + proxy components");
  }

  const provider = new JsonRpcProvider(rpcUrl);
  const network = await provider.getNetwork();
  if (network.chainId !== 224422n) {
    fail(`RPC ${rpcUrl} returned chainId ${network.chainId}, expected 224422`);
  }

  const byKey = new Map(record.components.map((component) => [component.key, component]));
  const preflighted = [];
  for (const target of record.components) {
    target.address = ensureAddress(target.address, target.key);
    const { code, tail } = await probeSolcTail(provider, target.address);
    if (code === "0x") fail(`${target.key} has no code at ${target.address}`);
    console.log(`chain solc tail ${target.key}: ${tail}`);

    const { buildInfo } = loadExactBuildInfo(target);
    const standardJson = pruneFullBuildInfo(buildInfo.input, target.sourceKey);
    const outputPath = path.join(root, "deployments", `conet-${target.key}-verify-buildinfo.json`);
    writeJson(outputPath, standardJson);
    ...
    const implementationAddress = target.kind === "proxy" ? byKey.get(target.implementationKey)?.address : null;
    if (!bytecodeMatchesAllowingAddressThis(localRuntime, code, target.address, implementationAddress)) {
  ...
  if (!submit) {
    ...
    console.log("Read-only preflight complete. Re-run with --submit to verify.");
    return;
  }
