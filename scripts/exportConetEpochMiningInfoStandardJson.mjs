#!/usr/bin/env node
/**
 * 导出 CoNET epoch_mining_info 验证用最小 Solidity Standard JSON Input。
 *
 * 前置: npm run compile
 *
 * 用法:
 *   node scripts/exportConetEpochMiningInfoStandardJson.mjs
 *
 * 输出: deployments/conet-epoch_mining_info-standard-input.json
 */
...
const sourceKey = "project/src/b-unit/mining_info.sol";
...
if (!buildInfoPath) {
  console.error("未找到含 mining_info.sol 的 build-info，请先: npm run compile");
  process.exit(1);
}
...
const outPath = path.join(root, "deployments", "conet-epoch_mining_info-standard-input.json");
...
console.log("Explorer 合约名（contract_name）: epoch_mining_info");
