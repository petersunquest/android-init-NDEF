...
/**
 * 将 Settle_ContractPool 使用的钱包（masterSetup.settle_contractAdmin 对应的地址）
 * 添加到两个 Factory 作为 paymaster，以便这些钱包可代表执行合约。
 * 调用方需为当前 AA Factory 的 admin 且 Card Factory 的 owner（通常用第一个 settle_contractAdmin 私钥）。
 */
export const addSettleWalletsToFactoriesAsPaymasters = async (): Promise<{ card: string[]; aa: string[] }> => {
	const adminPks = masterSetup.settle_contractAdmin
	if (adminPks.length === 0) {
		logger(Colors.yellow('[Beamio] settle_contractAdmin 为空，跳过'))
		return { card: [], aa: [] }
	}
	const provider = providerBaseBackup1
	const wallet = new ethers.Wallet(adminPks[0], provider)
	const addresses = adminPks.map((pk: string) => new ethers.Wallet(pk, provider).address)

	const cardFactory = new ethers.Contract(BeamioUserCardFactoryPaymasterV2, BeamioFactoryPaymasterABI, wallet)
	const aaFactory = new ethers.Contract(BeamioAAAccountFactoryPaymaster, BeamioAAAccountFactoryPaymasterABI, wallet)
	...
	for (const addr of addresses) {
		...
		try {
			...
			if (!isPaymaster) {
				...
				await tx.wait()
				addedCard.push(addr)
				logger(Colors.green(`[Beamio] Card Factory: 已添加 paymaster ${addr}`))
			} else {
				logger(`[Beamio] Card Factory: ${addr} 已是 paymaster`)
			}
		} catch (e: unknown) {
			const msg = e instanceof Error ? e.message : String(e)
			logger(Colors.red(`[Beamio] Card Factory 添加 ${addr} 失败: ${msg}`))
		}
		try {
			...
			if (!isPM) {
				...
				await tx.wait()
				addedAA.push(addr)
				logger(Colors.green(`[Beamio] AA Factory: 已添加 paymaster ${addr}`))
			} else {
				logger(`[Beamio] AA Factory: ${addr} 已是 paymaster`)
			}
		} catch (e: unknown) {
			const msg = e instanceof Error ? e.message : String(e)
			logger(Colors.red(`[Beamio] AA Factory 添加 ${addr} 失败: ${msg}`))
		}
	}
	logger(Colors.green(`[Beamio] 完成: Card 新增 ${addedCard.length} 个, AA 新增 ${addedAA.length} 个 paymaster`))
	return { card: addedCard, aa: addedAA }
}
...
const DeployingSmartAccount = async (wallet: string, SC: ethers.Contract): Promise<{ accountAddress: string, alreadyExisted: boolean }> => {
	...
	try {
		...
		if (nextIndex > 0n) {
			...
			const provider = (SC.runner as ethers.Wallet)?.provider ?? (SC as unknown as { provider?: ethers.Provider }).provider
			if (provider) {
...
const ACTION_FORWARD_1155 = 2

const makePayMeForForward1155ERC3009SignatureData = async (obj: forward1155ERC3009SignatureDataParams): Promise<PayMe> => {
	return {
		currency: 'USDC',
...
export const forward1155ERC3009SignatureDataProcess = async () => {
	...
	try {
		...
		const payMe = await makePayMeForForward1155ERC3009SignatureData(obj);
		const input = {
			actionType: ACTION_FORWARD_1155,
			card: obj.cardAddress,
			from: ethers.ZeroAddress,
			to: obj.fromEOA,
		...
		// TODO: persist or forward `input` (e.g. to Task indexer)
	} catch (error: any) {
