/**
 * Blockscout verify ChargeRewardModuleV2 + AdminStatsQueryModuleV6 from
 * conet-ChargeReward13Convert.json.
 *
 * Prerequisites:
 ...
 *   CONET_VERIFY_POLL_MAX=180 npx tsx scripts/verifyChargeReward13ConvertConet.ts
 *
 * Optional: CONET_VERIFY_ONLY=ChargeRewardModuleV2|AdminStatsQueryModuleV6
 */
import fs from 'fs'
...
const snapPath = path.join(root, 'deployments/conet-ChargeReward13Convert.json')
if (!fs.existsSync(snapPath)) {
	throw new Error(`Missing ${snapPath} — run upgradeChargeReward13ConvertConet.ts first`)
}
...
const V5_ADDR =
	snap.modules.adminStatsQueryModuleV5 || snap.constructorArgs?.v5 || ''
const VIEWS_ADDR =
	snap.modules.adminStatsReferrerViews || snap.constructorArgs?.referrerViews || ''
...
/** V6 constructor immutables (v5, referrerViews); map AST ids via on-chain sample. */
function patchV6Immutables(
	localHex: string,
	immutableReferences: LocalBytecodeResult['immutableReferences'],
	onchainHex: string,
): string {
	if (!immutableReferences || !V5_ADDR || !VIEWS_ADDR) return localHex
	const v5Word = addressAsImmutableWord(V5_ADDR)
	const viewsWord = addressAsImmutableWord(VIEWS_ADDR)
	const v5Bare = V5_ADDR.replace(/^0x/, '').toLowerCase()
	const viewsBare = VIEWS_ADDR.replace(/^0x/, '').toLowerCase()
...
async function isVerified(addr: string): Promise<{
	ok: boolean
	is_verified?: boolean
	...
	// Only trust Blockscout verification flags — source_code alone can be
	// incomplete / name-only and must not skip submit (conet-mainnet-blockscout-verify).
	const ok = Boolean(d.is_verified || d.is_partially_verified)
	return { ok, is_verified: d.is_verified, is_partially_verified: d.is_partially_verified }
}

function assertLocalMatchesChain(
	t: Target,
	localIn: string,
	onchain: string,
	immutableReferences?: LocalBytecodeResult['immutableReferences'],
	...
	if (t.key === 'AdminStatsQueryModuleV6') {
		local = patchV6Immutables(local, immutableReferences, onchain)
	}
...
async function verifyOne(t: Target): Promise<void> {
	console.log(`\n=== ${t.key} ${t.address} ===`)
	const status = await isVerified(t.address)
	if (status.ok) {
		console.log(
			`already verified (is_verified=${status.is_verified} partial=${status.is_partially_verified})`,
		)
	...
	if (localRes.bytecode) {
		assertLocalMatchesChain(t, localRes.bytecode, onchain, localRes.immutableReferences)
	}
	...
	for (let i = 0; i < max; i++) {
		...
		if ((await isVerified(t.address)).ok) {
...
async function main() {
	...
	if (!list.length) throw new Error(`CONET_VERIFY_ONLY matched nothing: ${only.join(',')}`)
	for (const t of list) await verifyOne(t)
	console.log('\nAll done')
}
