#!/usr/bin/env bash
# Manual 288 staking validators on 216.225.202.23 — NO Guardian / claimRedeem / mining-pool slot binding.
# NO contract upgrade: withdrawNative (contract admin) + direct beacon deposit() txs.
#
# Model:
#   - NEW validator keys on 23 (do not reuse legacy exited keys)
...
HOST23="${VALIDATOR_HOST_23:-216.225.202.23}"
...
echo "==> [1/6] Rsync validator-node scripts to ${HOST23}"
rsync -av \
	...
	"$REPO_ROOT/scripts/validator-node/setConetLabMiningPoolFeeRecipient23.sh" \
	...
	"${SSH23}:${NEWCONET}/"
ssh "$SSH23" "chmod +x '${NEWCONET}/01_generate_append_validator_deposits_listener.sh' '${NEWCONET}/08_import_append_validator_keys.sh' '${NEWCONET}/setConetLabMiningPoolFeeRecipient23.sh' '${NEWCONET}/batchExitValidatorsFromDeposits.sh' '${NEWCONET}/06_exit_validator.sh'"

echo "==> [2/6] Voluntary-exit legacy validators on ${HOST23} (if VA still uses old keys)"
ssh "$SSH23" bash -s -- "$NEWCONET" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
cd "$NEWCONET"
...
echo "==> [3/6] Generate ${COUNT} NEW validator keys (wc=ValidatorDepositRedeem) on ${HOST23}"
ssh "$SSH23" bash -s -- "$NEWCONET" "$REDEEM" "$COUNT" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
REDEEM="$2"
COUNT="$3"
...
VALIDATOR_COUNT="$COUNT" VALIDATOR_HOST_74="$HOST23" \
	node "$REPO_ROOT/scripts/depositManual136BeaconFromRedeemBalance.mjs"

echo "==> [5/6] Import keystores on ${HOST23}"
ssh "$SSH23" bash -s -- "$NEWCONET" "$POOL" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
POOL="$2"
cd "$NEWCONET"
...
echo "==> [6/6] proposer-settings (ConetLabMiningPool) + restart VA on ${HOST23}"
ssh "$SSH23" bash -s -- "$NEWCONET" "$POOL" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
POOL="$2"
...
FEE_RECIPIENT="$POOL" BEACON_RPC_PROVIDER=127.0.0.1:4000 \
	./setConetLabMiningPoolFeeRecipient23.sh
REMOTE

echo "==> Done. 23 runs manual ${COUNT} VA: Redeem wc, ConetLabMiningPool fee_recipient, no guardian ledger."
