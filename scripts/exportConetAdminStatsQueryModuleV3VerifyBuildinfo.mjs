#!/usr/bin/env node
/**
 * Generate deployments/conet-AdminStatsQueryModuleV3-verify-buildinfo.json + local bytecode precheck.
 *
 * Run after:
...
console.log(`\n${target.label}…`);
const std = pruneFullJson(target.fullRel, target.sourceKey);
const outPath = path.join(root, target.outRel);
fs.writeFileSync(outPath, JSON.stringify(std), "utf-8");
const kb = (fs.statSync(outPath).size / 1024).toFixed(1);
console.log(`  wrote ${target.outRel} (${kb} KB, ${Object.keys(std.sources).length} sources)`);
const local = compileDeployed(std, target.sourceKey, target.contractName);
const chain = chainCode(target.address);
if (local !== chain) {
  throw new Error(`${target.label} bytecode mismatch local=${local.length} chain=${chain.length}`);
}
console.log(`  ✅ bytecode match @ ${target.address}`);
