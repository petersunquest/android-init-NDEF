/**
 * Blockscout v2 verify for protocol USDC settlement mint modules:
 * IssuedNftModuleV2 + AdminStatsQueryModuleV4
 *
 * Prereq:
 ...
 *
 * Usage:
 *   npx tsx scripts/verifyProtocolUsdcSettlementMintConet.ts
 */
import fs from 'fs'
...
const snap = JSON.parse(
	fs.readFileSync(path.join(root, 'deployments/conet-ProtocolUsdcSettlementMint.json'), 'utf-8'),
) as {
	modules: { issuedNftModule: string; adminStatsQueryModule: string }
}
...
const TARGETS: Target[] = [
	{
		key: 'IssuedNftModuleV2',
		address: snap.modules.issuedNftModule,
...
async function verifyOne(t: Target): Promise<void> {
	...
	if (local) {
		...
		console.log('local bytecode matches eth_getCode')
	}
	...
	console.log(`\npoll timeout ${t.address}`)
	throw new Error(`${t.key} verification poll timeout`)
}
