#!/usr/bin/env bash
# Manual 288 staking validators on 216.225.202.22 — NO Guardian / claimRedeem / mining-pool slot binding.
# Same model as executeManual136StakingVaOn74.sh:
#   exit legacy → new keys → withdrawNative + deposit() → import → ConetLabMiningPool fee_recipient
#
set -euo pipefail
...
HOST22="${VALIDATOR_HOST_22:-216.225.202.22}"
...
echo "==> [1/6] Rsync validator-node scripts to ${HOST22}"
rsync -av \
	...
	"${SSH22}:${NEWCONET}/"
ssh "$SSH22" "chmod +x '${NEWCONET}/01_generate_append_validator_deposits_listener.sh' '${NEWCONET}/08_import_append_validator_keys.sh' '${NEWCONET}/setConetLabMiningPoolFeeRecipient74.sh' '${NEWCONET}/batchExitValidatorsFromDeposits.sh' '${NEWCONET}/06_exit_validator.sh'"

echo "==> [2/6] Voluntary-exit legacy ${COUNT} validators on ${HOST22}"
ssh "$SSH22" bash -s -- "$NEWCONET" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
cd "$NEWCONET"
...
echo "==> [3/6] Generate ${COUNT} NEW validator keys (wc=ValidatorDepositRedeem) on ${HOST22}"
ssh "$SSH22" bash -s -- "$NEWCONET" "$REDEEM" "$COUNT" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
REDEEM="$2"
COUNT="$3"
...
VALIDATOR_COUNT="$COUNT" VALIDATOR_HOST="$HOST22" \
	node "$REPO_ROOT/scripts/depositManual136BeaconFromRedeemBalance.mjs"

echo "==> [5/6] Import keystores on ${HOST22}"
ssh "$SSH22" bash -s -- "$NEWCONET" "$POOL" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
POOL="$2"
cd "$NEWCONET"
...
echo "==> [6/6] proposer-settings (ConetLabMiningPool) + restart VA on ${HOST22}"
ssh "$SSH22" bash -s -- "$NEWCONET" "$POOL" <<'REMOTE'
set -euo pipefail
NEWCONET="$1"
POOL="$2"
...
echo "==> Done. ${HOST22} runs manual ${COUNT} VA: Redeem wc, ConetLabMiningPool fee_recipient, no guardian ledger."
