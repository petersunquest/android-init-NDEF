...
async function main(): Promise<void> {
	...
	if (fs.existsSync(membershipSnapPath)) {
		...
		membership.combinedRewardSettersUpgrade = {
			...
			adminStatsQueryModule: routerAddr,
			chargeRewardModule: chargeAddr,
			note: 'V6 routes topupReward/chargeReward → ChargeReward; V5 + referrerViews unchanged',
		}
	...
	try {
		...
		const sel = ethers.id('recordTopupCumulativeStat(address,uint256)').slice(0, 10)
		const kind = Number(await card.selectorModuleKind(sel))
		console.log(`[smoke] card ${SMOKE_CARD} recordTopupCumulativeStat → route ${kind}`)
	} catch (e) {
	...
	console.log('  CONET_VERIFY_POLL_MAX=180 npx tsx scripts/verifyChargeRewardTopupRatioConet.ts')
	console.log(
