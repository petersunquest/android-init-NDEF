#!/usr/bin/env bash
# Register CONET-USDC metadata and icon in the self-hosted CoNET Blockscout.
#
# Run after the token proxy exists:
#   bash scripts/registerConetUsdcBlockscoutConet.sh
set -euo pipefail

HOST="${CONET_USDC_BLOCKSCOUT_HOST:-38.102.126.30}"
ADDR="${CONET_USDC_ADDRESS:-0xF9240fd613C00d5C479f1E9f1690130c5Fdc8BC3}"
ADDR_HEX="${ADDR#0x}"
ADDR_HEX="$(echo "$ADDR_HEX" | tr '[:upper:]' '[:lower:]')"
ICON_URL="${CONET_USDC_TOKEN_ICON_URL:-https://mainnet.conet.network/usdc/erc20/USDC-256.png}"
...
ON CONFLICT (contract_address_hash) DO UPDATE SET
  ...
  cataloged = true, icon_url = EXCLUDED.icon_url,
