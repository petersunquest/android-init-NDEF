#!/bin/bash
# Set execution-layer fee_recipient → ConetLabMiningPool for all validators on 216.225.202.23.
# Beacon runs locally on 23 (:4000); only restarts Prysm validator — not geth/beacon.
#
# Run ON 216.225.202.23:
#   FEE_RECIPIENT=0x32bE583C8e778FFfC5107BF34820c2B225336201 \
#   BEACON_RPC_PROVIDER=127.0.0.1:4000 \
#     bash setConetLabMiningPoolFeeRecipient23.sh
...
BEACON_RPC_PROVIDER="${BEACON_RPC_PROVIDER:-127.0.0.1:4000}"
PRYSM_VALIDATOR_BINARY="${PRYSM_VALIDATOR_BINARY:-$PROJECT_DIR/dependencies/prysm-v7.1.5/validator}"
[[ -x "$PRYSM_VALIDATOR_BINARY" ]] || PRYSM_VALIDATOR_BINARY="$PROJECT_DIR/dependencies/prysm-v7.1.4/validator"
...
pkill -f "$PRYSM_VALIDATOR_BINARY.*validator" 2>/dev/null || true
...
nohup "$PRYSM_VALIDATOR_BINARY" \
  ...
  --graffiti=conet-manual-va-23 \
  --chain-config-file="$CHAIN_CONFIG_FILE" \
  --suggested-fee-recipient="$FEE_RECIPIENT" \
  --proposer-settings-file="$PROPOSER_FILE" \
  >>"$NODE_DIR/logs/validator-manual23.log" 2>&1 &
echo $! >"$VALIDATOR_PID_FILE"
echo "validator pid=$(cat "$VALIDATOR_PID_FILE")"
