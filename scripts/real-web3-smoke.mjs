...
const ROUTE_KEY_ID = '9977E9A45187DD80'
const ENTRY = 'http://20ab90fe82d0e9e3.conet.network'
...
const officialKey = new ethers.Contract(
  ADDRESS_PGP,
...
const userKeyId = (keyIds[1] ?? keyIds[0]).toHex().toUpperCase()

await registerRoute()
const route = await searchKey(wallet.address)
...
const responsePromise = listenForResponse(listenController.signal)
await new Promise((resolve) => setTimeout(resolve, 1_000))
await sendRequest(official.userPublicKeyArmored)
...
console.log(JSON.stringify({
  ...
  gatewaySha256: gatewayHash,
  directSha256: directHash,
...
async function registerRoute() {
  ...
  const result = await fetch(REGISTER, {
    ...
    body: JSON.stringify({
      ...
      keyID: userKeyId,
      publicKeyArmored: Buffer.from(generated.publicKey, 'utf8').toString('base64'),
      encrypKeyArmored: encrypted,
      routeKeyID: ROUTE_KEY_ID,
    }),
  })
  const body = await result.text()
  if (!result.ok) throw new Error(`regiestChatRoute HTTP ${result.status}: ${body.slice(0, 120)}`)
}

async function searchKey(address) {
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const row = await officialKey.searchKey(ethers.getAddress(address))
    const userArmor = decodeArmor(String(row[1]))
    const routeArmor = decodeArmor(String(row[3]))
    if (userArmor.includes('BEGIN PGP') && routeArmor.includes('BEGIN PGP')) {
      return { userPublicKeyArmored: userArmor, routePublicKeyArmored: routeArmor }
    }
    await new Promise((resolve) => setTimeout(resolve, 1_000))
  }
  throw new Error(`searchKey did not become available for ${address}`)
}

async function sendRequest(officialUserArmor) {
  const request = {
    v: 1,
  ...
  const plaintext = JSON.stringify({ request, signMessage })
  const recipient = await openpgp.readKey({ armoredKey: officialUserArmor })
  const message = await openpgp.createMessage({ text: plaintext })
  const armor = await openpgp.encrypt({ message, encryptionKeys: recipient, format: 'armored' })
...
async function listenForResponse(signal) {
  ...
  const envelope = Buffer.from(JSON.stringify({ message: command, signMessage: signature })).toString('base64')
  const routeKey = await openpgp.readKey({ armoredKey: route.routePublicKeyArmored })
  const message = await openpgp.createMessage({ text: envelope })
  ...
  for await (const chunk of result.body) {
    ...
    const match = buffer.match(/-----BEGIN PGP MESSAGE-----[\s\S]*?-----END PGP MESSAGE-----/)
    if (!match) continue
    const encrypted = await openpgp.readMessage({ armoredMessage: match[0] })
