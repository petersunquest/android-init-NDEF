/**
 * CoNET Blockscout v2 standard-input verification for BeamioUserCardUpdateLib.
 *
 * Prerequisite:
 ...
 *
 * Run:
 *   CONET_VERIFY_POLL_MAX=180 npx tsx scripts/verifyConetUpdateLibOnScan.ts
 */
...
const VERIFY_JSON = path.join(root, "deployments/conet-UpdateLib-verify-buildinfo.json");
const CONTRACT_NAME = "project/src/BeamioUserCard/BeamioUserCardUpdateLib.sol:BeamioUserCardUpdateLib";
...
function loadAddress(): string {
  ...
  const raw = data.beamioUserCardUpdateLib;
  if (!raw) throw new Error("beamioUserCardUpdateLib missing in conet-addresses.json");
...
async function submit(address: string): Promise<void> {
  if (!fs.existsSync(VERIFY_JSON)) {
    throw new Error(`Missing ${VERIFY_JSON} — run exportConetUpdateLibVerifyBuildinfo.mjs first`);
  }
  const standardJson = JSON.parse(fs.readFileSync(VERIFY_JSON, "utf-8")) as object;
  const json = JSON.stringify(standardJson);
  console.log(`POST UpdateLib @ ${address} (json=${json.length} bytes)`);
...
async function main() {
  ...
  console.log("CoNET UpdateLib verify @", address);
