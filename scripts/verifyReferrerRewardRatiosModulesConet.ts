/**
 * Blockscout v2 verify for dual-referrer ratio upgrade stack:
 * ReferrerLib + ChargeRewardModuleV2 + AdminStatsQueryModuleV4
 *
 * Prereq:
 ...
 *
 * Usage:
 *   npx tsx scripts/verifyReferrerRewardRatiosModulesConet.ts
 */
import fs from 'fs'
...
const snap = JSON.parse(
	fs.readFileSync(path.join(root, 'deployments/conet-ReferrerRewardRatiosModules.json'), 'utf-8'),
) as {
	modules: { referrerLib: string; chargeRewardModule: string; adminStatsQueryModule: string }
	libraryLinks: {
