/**
 * CoNET Blockscout 验证 GBToken UUPS 栈（impl Standard JSON + proxy legacy partial）。
 *
 * 运行:
 *   npx tsx scripts/exportGBTokenStandardJson.ts
 *   npx tsx scripts/verifyErc20UupsGBConet.ts
 */
...
const IMPL_CONTRACT = "project/src/b-unit/GBToken.sol:GBToken";
...
async function verifyImpl(impl: string): Promise<void> {
  ...
  if (!fs.existsSync(jsonPath)) {
    throw new Error(`缺少 ${jsonPath}，请先 npx tsx scripts/exportGBTokenStandardJson.ts`);
  }
...
async function verifyProxy(proxy: string, impl: string): Promise<void> {
  ...
  const initData = iface.encodeFunctionData("initialize", [GBTOKEN_INITIAL_ADMIN]);
  ...
  if (!fs.existsSync(ozJson)) {
    console.warn("缺少 oz-ERC1967Proxy-standard-input.json；proxy 可 Read as Proxy 查看 impl");
...
async function main() {
  const impl = getAddress(process.env.GB_UUPS_IMPL || GBTOKEN_UUPS_IMPL_PREDICTED);
  const proxy = getAddress(process.env.GB_UUPS_PROXY || GBTOKEN_UUPS_PROXY_PREDICTED);
  console.log("impl:", impl);
  console.log("proxy:", proxy);
  await verifyImpl(impl);
  await verifyProxy(proxy, impl);
  console.log(`Explorer: ${BLOCKSCOUT_UI}/token/${proxy}`);
}
