...
- **日期：** 2026-08-20（MVP round 3 写回）
- **状态：** **代码已落地（mockL1Only）。** Round 1：本地 fixture + EventIngress + CLI + Explorer 只读。**Round 2：** Archive RPC/hook custody、`mock-auction-demo`、Explorer 会话密钥签名、根仓 `dle:deploy:mock-auction-local`。**Round 3：** Archive 真上链 `settle`（`mockL1Settle.ts`）、`mock-auction-e2e` / `dle:mock-auction-e2e`。**不是** CoNET 224422 现网接线，**不是** 生产 CL RANDAO / DePIN gossip。
...
- Client：`daemon/mock-l1-auction-cli.ts`、`daemon/mock-l1-auction-demo.ts`、`daemon/mock-l1-auction-e2e.ts`；Explorer `/mock-auction` + `explorer/src/lib/mockAuctionWire.ts`
...
6. **Round 3 settle：** Archive 提交真实 local-RPC `settle`；Explorer 只展示 timeline / `settlementTxHash`，不代签 authority。
...
- Explorer 一键触发 Archive on-chain settle（可选 Round 4）
