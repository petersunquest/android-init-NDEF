# conet-l0d：crate MVP 验收 + P1 出站 / 入站解密（crate 内；实验室二进制 `[l0]` 关）
...
**快照日期：** 2026-08-17  
**状态：** crate MVP **已验收**；P1 出站 **已在 crate**；P1 入站解密 + TUN 写回 **已在 crate**；`[l0]` **默认关**；实验室二进制将再装一次且 **不**开 `[l0]`；**未**打开现役 mailbox SSE；**未**改 advertise 为 vIP；**未**改 SI。`.98` geth 已于 **21:47:24Z** 由既有 load watchdog 拉起（早先 21:40Z 因 `load15>2.11` 停机，**不是** l0d 导致）。

## 目标

1. 诚实验收本阶段：crate 生命周期 + 两机公网互为 peer + overlay TUN 桩。
2. 按白皮书 §10 推进 **P1**：在现役 `/post` + OpenPGP + A/B/C 上做钱包对钱包 TCP 字节流，而不是新 SI 命令。
3. 同一任务把「crate 已有出站 encrypt+wrap+POST **以及**入站解密+TUN 写回 / 未打开现役 mailbox SSE / 实验室可装二进制且 `[l0]` 关」写回白皮书、P1 成对页、RULES、GitBook Applications + Developers，并按用户要求 git push + 部署。
...
| `.50` / 本机 `cargo test` + `cargo build --release` | 39 passed（34 unit + 5 integration）；release 将再编 | **真实实测** |
...
**不是实测：** overlay TCP 送达、生产 SI mailbox 往返、现役 mailbox SSE、L0 hop RTT、EL 追上 tip。
...
确定性关系（P1 出站 + 入站解密，已在 crate；现役 mailbox SSE 未打开）：
...
HTTP 只能 `{ "data" }`。`NoPush` 只能进 B route PGP 工作包。出站 fail-closed：`[l0].enabled` + 对端 user PGP + 对端 route PGP + 至少一个 entry。入站 fail-closed：`[l0].enabled` + `routing_key_file` 为 OpenPGP **私钥**证书。
...
3. **不得声称：** overlay 已转发 TCP；advertise 已切 vIP；EL 已同步；存在现役 SI `p2p_stream_*` / `listenKind: l1p2p`；生产 mailbox 已投递；现役 mailbox SSE 已打开。
4. **P1 出站 + 入站解密/TUN 写回已在 crate。** `cargo test` 39。装二进制 ≠ 现役 mailbox 客户端。**未**打开现役 mailbox SSE。
...
- 打开现役 mailbox SSE（专用 routing EOA；经 C ≠ B；EIP-191 签名 listen）。
...
- [x] 入站解密 + TUN 写回队列（crate 内；不打开现役 SSE）
- [x] `cargo test` 39（34 unit + 5 integration）
