...
**快照日期：** 2026-08-17  
**状态：** crate MVP **已验收**；P1 出站 **已在 crate**；P1 入站解密 + TUN 写回 **已在 crate**；P1 EIP-191 listen HTTP+SSE worker **已在 crate**（mock 测过；线形状对齐 SI `checkSign`）；`[l0]` **默认关**；实验室二进制将再装一次且 **不**开 `[l0]`；**未**打开生产 SI listen；**未**改 advertise 为 vIP；**未**改 SI。`.98` geth 已于 **21:47:24Z** 由既有 load watchdog 拉起（早先 21:40Z 因 `load15>2.11` 停机，**不是** l0d 导致）。

## 目标

1. 诚实验收本阶段：crate 生命周期 + 两机公网互为 peer + overlay TUN 桩。
2. 按白皮书 §10 推进 **P1**：在现役 `/post` + OpenPGP + A/B/C 上做钱包对钱包 TCP 字节流，而不是新 SI 命令。
3. 同一任务把「crate 已有出站 encrypt+wrap+POST、入站解密+TUN 写回、**以及** EIP-191 listen HTTP+SSE worker（mock；对齐 `checkSign`）/ 未打开生产 SI listen / 实验室可装二进制且 `[l0]` 关」写回白皮书、P1 成对页、RULES、GitBook Applications + Developers，并按用户要求 git push + 部署。
...
| `.50` / 本机 `cargo test` + `cargo build --release` | 本档 `cargo test` **47** passed（42 unit + 5 integration）；release 将再编 | **真实实测** |
...
Listen（crate 内；mock 测过；EIP-191 + SI wrap）:
{ command: mining, listenKind: chat, walletAddress, timestamp }
  → EIP-191 personal_sign → { message, signMessage } → standard base64
  → encrypt(this host B route PGP) → POST { "data" } to C ≠ B
  → SSE chunk → extract armor → apply_inbound_armor
```

HTTP 只能 `{ "data" }`。`NoPush` 只能进 B route PGP 工作包。出站 fail-closed：`[l0].enabled` + 对端 user PGP + 对端 route PGP + 至少一个 entry。入站 fail-closed：`[l0].enabled` + `routing_key_file` 为 OpenPGP **私钥**证书。Listen fail-closed：enabled + `listen_entries`（禁止回退出站 `entries`）+ `mailbox_route_pgp_file`（本机 B route **公钥**）+ `routing_eoa` + `routing_key_file` + `routing_eth_key_file`（hex secp256k1；recovered 地址须等于 `routing_eoa`；不是 OpenPGP）。Listen 为 EIP-191 + SI `{ message, signMessage }` base64。crate 内线形状对齐 `checkSign`。**未**打开生产 SI listen。
...
4. **P1 出站 + 入站解密/TUN 写回 + EIP-191 listen HTTP+SSE worker 已在 crate。** `cargo test` 47（42 unit + 5 integration）。crate 内线形状对齐 SI `checkSign`。装二进制 ≠ 现役 mailbox 客户端。**未**打开生产 SI listen。
...
- 生产 SI listen（crate 内已 EIP-191 签 `mining`；专用 routing EOA；经 C ≠ B）。**未**打开：`[l0]` 关；不向生产 SI POST。
...
- [x] Listen HTTP+SSE worker（crate 内；wiremock；EIP-191 + SI `{ message, signMessage }`）
- [x] `cargo test` 47（42 unit + 5 integration）
