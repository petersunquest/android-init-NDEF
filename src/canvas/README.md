...
| [DLE MVP 里程碑评估（2026-08-17）](./dle-mvp-milestone-assessment-2026-08-17.md) | `dle-mvp-milestone-assessment-2026-08-17.canvas.tsx` | **2026-08-17 审查：控制面 live / 诚实轨引擎 154/154 / P23 keep-deploy 6/7 + 409→accept / P24 `node.ts` 已接线 / P25 Explorer overlay + 入座文案 lab EIP-712 + 公开 SPA `index-U1o9ul_I.js` 已发 / fd-06 HTTP 不稳 / 资格未开** | 替代 08-15 评估为当前审查；P23/P24/P25 已落地（诚实 6/7；隔离 `node.ts` 门；Explorer 非绿 overlay + 入座文案 EIP-712 + 公开 SPA）；下一闸停放 / 仅审查；不得开钟；不得宣称 7/7；不得发明 P26 |
| [DLE MVP：P11 之后的下一闸](./dle-mvp-p12-milestones-2026-08.md) | `dle-mvp-p12-milestones-2026-08.canvas.tsx` | **2026-08-17：P12–P25 已落地（引擎+单测 154/154；`explorer:test` 8/8；公开 SPA `index-U1o9ul_I.js`）** | 入座票、挑战/opening、BFT AC、on-demand attest、P6 \(Q_V\) 实验室 EIP-712；挑战与 on-demand 先冻后绑实验室 beacon（非 live CL RANDAO）；\(C_G\) 分轨（实验室 2249 ≠ L1 `archiveGroupId`）；gossip wait-hook 诚实闸已落地（**不是**生产 DePIN gossip）；P21 把 `hashIndexRoot` 写入实验室 BFT（树仍独立；**不是**生产 AC 承诺）；P22 官方 standby 实验室 EIP-712 就绪签（extra `fd-08` 不计；新链 accept 可 409；**不是**生产 OperatorDomain）；P24 隔离 `node.ts` 已接同一回调（不 `sync.start()` / 不冻库存）；P25 Explorer 非绿 overlay（绿点仍只 `seatingQualified`；入座文案 lab EIP-712；公开 SPA 已发）；不得开 `pilotStartedAt`；不改白皮书生产条款；不得发明 P26 |
...
| [DLE MVP 里程碑评估](./dle-mvp-milestone-assessment-2026-08.md) | `dle-mvp-milestone-assessment-2026-08.canvas.tsx` | **历史：2026-08-15 审查**（当前审查见 08-17 评估） | P0–P4 实验室完成、P5 16/16 验证；HMAC/HTTP/lab beacon 不得外推生产；warmup ~24/72h；`pilotStartedAt=null` |
