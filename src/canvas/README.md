...
| --- | --- | --- | --- |
| [Web3 Browser 产品模型](./web3-browser-product-2026-08.md) | `web3-browser-shell-pwa` / `web3-browser-cross-platform`（Cursor IDE） | **2026-08-23：Phase 0 脚手架 + 白皮书 EN/ZH + MVP + GitBook stub** | 原生壳+PWA；TS SI/host/L2；IndexedDB；Embedded OTA；能力④待定 |
| [DLE Mock-L1 拍卖撮合 MVP](./dle-mock-l1-auction-mvp-2026-08.md) | 无独立交互 Canvas | **2026-08-21：Round 1–11 已落地（+ CI Hardhat 全量 e2e + Explorer recovery 文案；一键 shell 先 unlist 回收再 settle）；非主网 / 非生产 RANDAO·DePIN** | EventIngress + certificate + 1bps 50/50；Archive RPC custody + `list`/`unlist`/`approve`/`preflight`/`settle`；Explorer Unlist / Mark failed→Unlist + Preflight + List→Approve→Settle；父仓/DLE CI gate；旁路 lab `notL1Nft` |
...
| [DLE 30 天冻结期中期评估（~46.75% · 2026-09-01）](./dle-30d-freeze-midwindow-assessment-2026-09-01.md) | `dle-30d-freeze-midwindow-assessment-2026-09-01.canvas.tsx` | **当前冻结期中期评估：10:35:23Z；正式 7/7 冻结 sticky·钟对齐·QUALIFIED·`pilotQualified=false`；正式 `lastQuorumOk` **2/7**（相对 Aug 20 6/7 恶化）；`fd-05` peer=0；leaf/ready omit；墙钟终点 ~2026-09-17** | 冻结 sticky + 开钟仍成立 ≠ 合格；quorum 仅可达性；缺字段 ≠ 空库存；不得发明 P26 |
| [DLE 运行评估（开钟后 ~45.8h · 2026-08-20）](./dle-mvp-runtime-review-2026-08-20.md) | `dle-mvp-runtime-review-2026-08-20.canvas.tsx` | **历史运行审查：07:43:59Z；正式 quorum 当时 6/7** | 开钟早期舰队实况；被 09-01 中期评估替代瞬时读数 |
| [DLE 运行评估（开钟后 ~13.8h · 2026-08-18）](./dle-mvp-runtime-review-2026-08-18.md) | `dle-mvp-runtime-review-2026-08-18.canvas.tsx` | **历史运行审查：23:41Z** | 开钟当日舰队实况；缺字段 ≠ 0；开钟 ≠ 合格 |
| [DLE 工作评估（2026-08-18）](./dle-mvp-work-review-2026-08-18.md) | `dle-mvp-work-review-2026-08-18.canvas.tsx` | **当前工作审查（控制面）：实验室闭环 / 诚实轨 / P23 历史 / 开钟戳** | 与运行/中期评估并列；开钟 ≠ 合格；下一闸见中期评估 |
| [DLE MVP 里程碑评估（2026-08-17）](./dle-mvp-milestone-assessment-2026-08-17.md) | `dle-mvp-milestone-assessment-2026-08-17.canvas.tsx` | **已归档。交互页曾停在开钟前，已纠正并指向 08-18 工作评估** | 历史对照；当前审查见上一行 |
| [DLE MVP：P11 之后的下一闸](./dle-mvp-p12-milestones-2026-08.md) | `dle-mvp-p12-milestones-2026-08.canvas.tsx` | **2026-08-18：P12–P25 已落地；钟已打 `pilotStartedAt=2026-08-18T09:53:58.092Z`；Explorer 钟 overlay 已发 `index-C8IdTq4H.js`（不是 P26）** | 入座票、挑战/opening、BFT AC、on-demand attest、P6 \(Q_V\) 实验室 EIP-712；挑战与 on-demand 先冻后绑实验室 beacon（非 live CL RANDAO）；\(C_G\) 分轨（实验室 2249 ≠ L1 `archiveGroupId`）；gossip wait-hook 诚实闸已落地（**不是**生产 DePIN gossip）；P21 把 `hashIndexRoot` 写入实验室 BFT（树仍独立；**不是**生产 AC 承诺）；P22 官方 standby 实验室 EIP-712 就绪签（extra `fd-08` 不计；新链 accept 可 409；**不是**生产 OperatorDomain）；P24 隔离 `node.ts` 已接同一回调（不 `sync.start()` / 不冻库存）；P25 Explorer 非绿 overlay（绿点仍只 `seatingQualified`）；开钟 ≠ 合格；不得发明 P26 |
...
| [DLE MVP 里程碑评估](./dle-mvp-milestone-assessment-2026-08.md) | `dle-mvp-milestone-assessment-2026-08.canvas.tsx` | **历史：2026-08-15 审查**（当前审查见 08-18 工作评估） | P0–P4 实验室完成、P5 16/16 验证；HMAC/HTTP/lab beacon 不得外推生产；warmup ~24/72h；当时 `pilotStartedAt=null` |
