...
/// POS shell loads the bundled PWA via `beamio-pos-local://` scheme handler.
...
private func shouldBlockBeamioNdefTopLevelNavigation(_ url: URL, isMainFrame: Bool) -> Bool {
    ...
    if host.contains("cashtrees.beamio.app") || host.contains("pos.beamio.app") { return false }
...
final class CashTreesWebCoordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, NFCTagReaderSessionDelegate {
    ...
    func loadWebAppURL(_ url: URL, in webView: WKWebView, bypassDedup: Bool = false) {
        let mapped = BeamioDeepLink.mapResolvedWebAppURLToLocal(url)
        let key = mapped.absoluteString
        if !bypassDedup, lastLoadedWebURLString == key { return }
        lastLoadedWebURLString = key
        var request = URLRequest(url: mapped)
    ...
    private func performWebContentRecovery(on webView: WKWebView) {
        ...
        loadWebAppURL(CashTreesLocalPWAHost.shared.entryURL, in: webView, bypassDedup: true)
    }
        ...
        (function(){
          ...
          window.CashTreesIOS={
            ...
            startStripePhysicalPayment:function(payload){
              ...
              window.webkit.messageHandlers[H].postMessage({
                ...
                buyerEoa:payload.buyerEoa||'',
                amountFiat6:payload.amountFiat6||'',
                currency:payload.currency||'',
                kind:payload.kind||'topup',
                businessIdempotencyKey:payload.businessIdempotencyKey||'',
            ...
            openURL:function(payload){
              payload=payload||{};
              var url=typeof payload==='string'?payload:(payload.url||'');
              window.webkit.messageHandlers[H].postMessage({
                action:'openURL',
                url:url
              });
            ...
            notifyBackgroundChat:function(payload){
              payload=payload||{};
              window.webkit.messageHandlers[H].postMessage({
                action:'notifyBackgroundChat',
                badge:payload.badge,
                appIconBadge:payload.appIconBadge,
                title:payload.title||'',
                body:payload.body||''
              });
            },
          ...
          window.BeamioPOS={
            ...
            postMessage:function(body){
              body=body||{};
              window.webkit.messageHandlers[H].postMessage(body);
            }
          };
        })();
        """
...
struct CashTreesWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        let initialURL = deepLinkStore.takePendingWebURL() ?? defaultLocalWebAppURL(from: localPWAHost)
