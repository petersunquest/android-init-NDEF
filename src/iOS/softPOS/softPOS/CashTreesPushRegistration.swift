...
//  Parity with Consumer CashTrees_iOS CashTreesPushRegistration.
//
...
enum CashTreesPushRegistration {
	private static let log = Logger(subsystem: "com.beamio.app.pos", category: "push")
	...
	static func requestAuthorizationAndRegister() {
		...
		center.requestAuthorization(options: [.badge, .alert, .sound]) { granted, _ in
			guard granted else {
				log.info("push auth denied or not granted")
				return
			}
	...
	static func payloadForWebEvent(deviceToken hex: String) -> [String: Any] {
		var payload: [String: Any] = [
			...
			"deviceToken": hex,
			"platform": "ios",
			"bundleId": Bundle.main.bundleIdentifier ?? "com.beamio.app.pos",
		]
