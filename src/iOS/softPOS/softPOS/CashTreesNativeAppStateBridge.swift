...
//  softPOS
//
//  PWA → Native app state (app icon badge, background chat local push).
//  Mirrors consumer CashTrees_iOS; independent copy (no cross-project import).
//
		...
		if let push = state?["backgroundChatNotify"] as? [String: Any] {
			...
			if present {
				presentBackgroundChatNotification(
					badge: badge,
					title: (title?.isEmpty == false ? title! : "Beamio POS"),
