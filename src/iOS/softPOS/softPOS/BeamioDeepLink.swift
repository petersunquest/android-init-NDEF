...
enum BeamioDeepLink {
    ...
    /// Map an allowed remote POS URL to the on-device embedded scheme (PUBLIC_URL=/).
    static func mapResolvedWebAppURLToLocal(_ remote: URL, localBase: URL = localWebAppBaseURL) -> URL {
        if remote.scheme?.lowercased() == CashTreesPWAScheme.scheme {
            return remote
        }
