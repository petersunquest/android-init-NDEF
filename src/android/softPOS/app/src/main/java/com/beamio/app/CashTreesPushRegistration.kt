 ...
 * Mirrors Consumer [CashTreesPushRegistration] and iOS softPOS CashTreesPushRegistration.swift.
 * Play package / FCM bundleId: [applicationId] = `com.beamio.pos`.
 */
object CashTreesPushRegistration {
    ...
    /** PWA `bindPushIdentity({ eoa, pgpKeyId? })` — JSON string via bridge. */
    ...
    fun bindIdentityFromJson(context: Context, json: String) {
        try {
            ...
            logNative("bindPushIdentity invalid json")
        }
    }
        ...
        try {
            FirebaseMessaging.getInstance().token
                ...
                .addOnFailureListener { e ->
                    logNative("FCM token failed: ${e.message}")
                }
        } catch (e: Exception) {
            logNative("FCM unavailable: ${e.message}")
        }
    }
    ...
    /** Call after POST_NOTIFICATIONS dialog — re-publish FCM token to PWA. */
    fun onPostNotificationsPermissionResult(context: Context, granted: Boolean) {
        if (!granted) {
            logNative("push POST_NOTIFICATIONS denied")
        }
        fetchAndPublishToken(context)
    }
