...
/** POS WebView shell — loads POS PWA only (see beamio-pos-pwa-native-webview-shell.mdc). Deprecated: android-NDEF native POS UI. */
/**
...
private object NfcStatusStrings {
    ...
    /** PWA expects `nfc_disabled` (see cashTreesScanBridge.ts). */
    const val DISABLED = "nfc_disabled"
...
class MainActivity : ComponentActivity() {
    ...
    private var nfcScanOverlay: NfcScanOverlayView? = null
    private var qrScanOverlay: QrScanOverlayView? = null
    private var pendingNativeQrAfterCameraGrant = false
    ...
    private val requestCameraPermission = registerForActivityResult(
        ...
        if (pendingNativeQrAfterCameraGrant) {
            pendingNativeQrAfterCameraGrant = false
            if (granted) {
                showQrScanOverlayInternal()
            } else {
                completeQrScanError("camera_permission_denied")
            }
    ...
    ) { uri -> qrScanOverlay?.onExternalImageUri(uri) }

    private val qrOpenDocumentLauncher = registerForActivityResult(
    ...
    private fun showNfcScanOverlay() {
        ...
        hideNfcScanOverlay()
        val overlay = NfcScanOverlayView(this) {
            disarmNfcReader(true, "cancelled")
        }
        nfcScanOverlay = overlay
        rootLayout.addView(
            overlay,
    ...
    private fun hideNfcScanOverlay() {
        if (!::rootLayout.isInitialized) return
        nfcScanOverlay?.let { rootLayout.removeView(it) }
        nfcScanOverlay = null
    }
    ...
    private fun launchGeneralQrScanner(requestId: String, bridgeAction: String, @Suppress("UNUSED_PARAMETER") filter: String) {
        ...
        runOnUiThread {
            ...
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                ...
                pendingNativeQrAfterCameraGrant = true
                requestCameraPermission.launch(Manifest.permission.CAMERA)
            }
        }
    }

    private fun showQrScanOverlayInternal() {
        if (!::rootLayout.isInitialized) return
        hideQrScanOverlay()
        val requestId = pendingQrScanRequestId.orEmpty()
        val bridgeAction = pendingQrScanAction
        val overlay = QrScanOverlayView(
            host = this,
            onSuccess = { text ->
                hideQrScanOverlay()
    ...
    private fun hideQrScanOverlay() {
        if (!::rootLayout.isInitialized) return
        qrScanOverlay?.release()
        qrScanOverlay?.let { rootLayout.removeView(it) }
        qrScanOverlay = null
    }

    private fun completeQrScanError(error: String) {
        val requestId = pendingQrScanRequestId.orEmpty()
        val bridgeAction = pendingQrScanAction
        pendingQrScanRequestId = null
        pendingQrScanAction = "scanQr"
    ...
    fun showQrChooseSourceDialog(overlay: QrScanOverlayView) {
        overlay.beginLibraryPick()
        androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Choose source")
            .setItems(arrayOf("Photo Library", "Browse Files")) { _, which ->
                when (which) {
                    0 -> qrPickImageLauncher.launch("image/*")
                    1 -> qrOpenDocumentLauncher.launch(arrayOf("image/*"))
                }
            }
            .setOnCancelListener { overlay.cancelLibraryPick() }
            .show()
    }
    ...
    private inner class CashTreesJsBridge {
        ...
        /**
         * PWA → Native generic app state (launcher icon badge).
         * Payload: JSON string `{ action:'publishAppState', state:{...} }` — mirrors iOS.
         */
        @JavascriptInterface
    ...
    private fun shouldBlockBeamioNdefTopLevelNavigation(u: Uri, isMainFrame: Boolean): Boolean {
        ...
        if (host.contains("pos.beamio.app")) return false
        ...
        return createBaseWebView(jsBridge).apply {
            webViewClient = object : WebViewClient() {
                override fun shouldInterceptRequest(
                    ...
                    host.mapPosUrlToLocal(u)?.let { local ->
