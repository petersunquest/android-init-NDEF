...
class Socks5 {
    ...
    init(client: ServerConnection) {
        self.client = client
        self.client.connection.receive(minimumIncompleteLength: 1, maximumLength: self.MTU) {(data1, _, isComplete, error) in
            ...
            if isComplete {
                return print("Local Proxy Socks5 \(client.port) connection isComplete!")
            }
    ...
    func send(data: Data) {
        self.client.connection.send(content: data, completion: .contentProcessed({ error in
            if let error = error {
                return self.client.connectionDidFail(error: error)
            }
        }))
    }
    ...
    var host = ""
    func CONNECT() {
        print("Local Proxy Socks5 \(client.port) got CONNECT: \(String(describing: self.rfc1928.CMD()))")
        ...
        self.client.connection.receive(minimumIncompleteLength: 1, maximumLength: self.MTU) {(data, _, isComplete, error) in
                ...
                Task{
                    ...
                    if let callFun2 = self.client.layerMinus.javascriptContext.objectForKeyedSubscript("json_sign_message") {
                        if let ret2 = callFun2.call(withArguments: [message, "0x\(signMessage.toHexString())"]) {
                            ...
                            let conBri = ServerBridge(sendData: request.data(using: .utf8)!, host: host, port: port, proxyConnect: self.client)
