...
import {ReferralRegistrySettlementLib} from "./ReferralRegistrySettlementLib.sol";
...
interface IReferralMerchantShareModule {
 ...
 * @notice Global Admin -> L0 -> L1/Merchant registry.
 *
 * Paid B-Unit burns mint CONET-USDC on BUnitAirdrop, then this vault splits the
 * L0/L1 rebate and **immediately** pays each beneficiary EOA via
 * `reserveClaimable` + `payoutClaimable` (no pending claim step).
 *
...
contract ReferralRegistryVaultV1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @dev ReferralMerchantShareModuleV1 proxy — L0 merchant rebate shares for L1s.
    address public merchantShareModule;
    ...
    event MerchantShareModuleUpdated(address indexed merchantShareModule);
    ...
    /// @notice Rebate USDC paid straight to beneficiary EOA (no claim queue).
    ...
    function setMerchantShareModule(address merchantShareModule_) external onlyOwner {
        merchantShareModule = merchantShareModule_;
        emit MerchantShareModuleUpdated(merchantShareModule_);
    }

    function creditClaimableFromShare(address account, uint256 amount, bytes32 sourceHash) external {
        ReferralRegistrySettlementLib.creditClaimableFromShare(
            claimedConetUsdc,
            merchantShareModule,
            bunitAirdrop,
            account,
    ...
    /**
     * @notice Drain legacy `claimableConetUsdc` (pre–direct-pay) to the beneficiary EOA.
     *         Requires Airdrop already holds reserved V3 USDC for this amount.
     */
    function flushPendingClaimable(address account) external {
        ReferralRegistrySettlementLib.flushPendingClaimable(
            claimableConetUsdc,
            claimedConetUsdc,
            claimNonces,
            bunitAirdrop,
    ...
    function onPaidBUnitConsumed(
        ...
        kind;
        ReferralRegistrySettlementLib.onPaidBUnitConsumed(
            _membersAsSettlementView(),
            claimedConetUsdc,
            bunitAirdrop,
            merchantShareModule,
            payer,
    ...
    function _membersAsSettlementView()
        private
        view
        returns (mapping(address => ReferralRegistrySettlementLib.MemberView) storage memberViews)
    {
        assembly ("memory-safe") {
            memberViews.slot := members.slot
        }
    }
}
