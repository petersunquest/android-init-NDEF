...
/// @dev External stats library entry (library delegatecall from proxy reverts; use CALL + immutable lib address).
interface IValidatorDepositRedeemUnifiedStats {
    ...
    ) external view returns (UnifiedIncomeStats memory);
}
...
contract ValidatorDepositRedeem is Initializable, UUPSUpgradeable {
    ...
    mapping(address => uint256) private clRewardPaid;
    ...
    /// @dev CL settle period buckets (UTC, aligned with ValidatorNodeRewardIndexer).
    mapping(address => mapping(uint256 => uint256)) private beneficiaryClHourly;
    mapping(address => mapping(uint8 => mapping(uint256 => uint256))) private beneficiaryClPeriod;
    /// @notice GBDepinAirdrop with paidGbSummaryOf period views (DePIN paid GB path).
    address private gbDepinAirdrop;
    ...
    modifier nonReentrantNative() {
        require(_nativeLock != 2, "ValidatorRedeem: reentrant");
        _nativeLock = 2;
        _;
        _nativeLock = 1;
    }
    ...
    /// @dev Deployed StatsLib address for {resolveUnifiedIncomeStats} (immutable per implementation).
    address private immutable _unifiedStatsLib;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address unifiedStatsLib_) {
        _unifiedStatsLib = unifiedStatsLib_;
    ...
    function setGbDepinAirdrop(address gbDepinAirdrop_) external onlyRedeemAdmin {
        gbDepinAirdrop = gbDepinAirdrop_;
    }
    ...
    {
        return
            IValidatorDepositRedeemUnifiedStats(_unifiedStatsLib).resolveUnifiedFromRedeem(
                address(this), maybeWallet, conetDepinNodeIp, anchorTs, gbDepinAirdrop
            );
    }
