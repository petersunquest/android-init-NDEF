 ...
 *
 * Compatibility (installed apps):
 * - Raw `PosWalletInitRecord` in IndexedDB (canonical since harden fix)
 * - PouchDB-style `{ title: base64(JSON) }` envelope
 * - localStorage fallback key `beamio_pos_wallet_init_v1`
 */
...
function parseStoredInit(raw: unknown): PosWalletInitRecord | null {
	/* Production installs store the raw record; some older builds used a base64 envelope. */
...
async function saveToIndexedDb(record: PosWalletInitRecord): Promise<void> {
	...
	try {
		/* Store raw record — matches installed apps since harden fix. */
