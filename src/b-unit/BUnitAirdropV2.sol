...
contract BUnitAirdropV2 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function consumeFromUser(
        ...
        uint256 usdcAmount = paidBurned / BUNIT_TO_USDC;
        totalPaidBunitConsumed += paidBurned;

        if (usdcAmount > 0) {
            IConetTreasuryV2(conetTreasury).mintForAdmin(conetUsdc, address(this), usdcAmount);
        ...
        emit PaidBUnitConsumed(user, paidBurned, usdcAmount, sourceHash, kind);
    }
