...
**重部署 Factory/BeamioUserCard 或新增共享 module 后必做：**
1. 更新 `config/base-addresses.ts` 和 `src/x402sdk/src/chainAddresses.ts` 的 CARD_FACTORY
2. 若本次包含新的共享 module（如 membership stats），先完成 module 部署并绑定到 `BeamioUserCardFactoryPaymasterV07`
3. `npm run compile && npm run sync:card-artifact`
4. **重启 Master 服务**（否则仍用旧 artifact/配置）
...
`BeamioUserCard` 会动态跟随工厂当前的 `default*Module`。因此只更新源码或 ABI 不够；若引入新的共享 module（例如 membership stats write/query），必须确保 Base 主网工厂已完成绑定，否则新发卡不会获得新能力。

**建议检查：**
```bash
npx hardhat run scripts/checkCreateCardDeployerConfig.ts --network base
```
